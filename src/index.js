...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const fetchNotes = async () => {
    try {
      const responseArchive = await fetch(`${baseUrl}/notes/archived`);
      const responseRegular = await fetch(`${baseUrl}/notes`);
      const responseArchiveJson = await responseArchive.json();
      const responseRegularJson = await responseRegular.json();
      const noNotesMessageSelector = document.body.querySelector('.no-notes-message');
      notesData = [...responseArchiveJson.data, ...responseRegularJson.data];
